// VideoSubtitle.cpp : 定义控制台应用程序的入口点。
...
void SaveFrame(uint8_t *pFrame, int width, int height, int video_id, char *videofile)
{
	...
	sprintf(szFilename, "%s_%dx%d.264\0",videofile,width,height);
...
int yuv_analysis(const char* yuvname)
{
	...
	while(!feof(pFile))
	{
		...
		//video_id = feature_match(pFrame,width, height);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	content_analysis2("D:\\视频\\1.mp4",1000000);
	return 0;
}
